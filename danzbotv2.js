...
prefix = ","
name = "~ DANZ BOT"
rdaftar = "TERIMA KASIH TELAH MENDAFTAR MENJADI MEMBER DANZBOT ^_^"
rmenu = "HAI MEMBER DANZBOT👋 JANGAN LUPA DONASI YAA:)"
botinfo = "UNTUK INVITE BOT KE DALAM GRUP, SILAHKAN DONASI DULU YAA:)"
limitt = 10
memberLimit = 2
ban = []
premium = ["6289686610207@s.whatsapp.net","6281253491026@s.whatsapp.net","6282111829914@s.whatsapp.net"]
...
async function starts() {
	...
	frhan.on('chat-update', async (mek) => {
       ...
       if (messagesLink.includes("://chat.whatsapp.com/")){
		...
		case 'daftar':
					frhan.updatePresence(from, Presence.composing)
					if (isUser) return reply('kamu sudah Menjadi Temen DANZBOT:D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
...
case 'asupan':
				...
				reply(mess.wait)
                 ...
                 frhan.sendMessage(from, asupan, video, {quoted: mek, caption: '\`\`\`ASUPAN GAN:b\`\`\`'})
				await limitAdd(sender) 
				break  
				case 'wiki':
				if (isBanned) return reply(mess.only.benned)    
